...
// Constantes para Intensidad mulinica
#define LDR A0 // Puerto análogo para lectura del voltaje
#define Rc 9820 // Resistencia de calibración en Ohms
#define Vin 3.3 // Voltaje de alimentación en voltios
#define RDark 500 // Resistencia en oscuridad en KΩ
#define ledPin 0 // Pin D4 como salida digital para encender el led
...
const char ssid[] = "Fmlia Castro Gonzalez"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "100056438544"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "18.215.33.88"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
float humi;
// Valor de la medición de la intensidad luminica
int lux;
...
/**
 * Lee la resistencia de y la convierte a Lux, la imprime en consola y la devuelve.
 */
int readLuminosidad(){
 int analog_value_ldr = analogRead(LDR);
 float Vout = float(analog_value_ldr) / float(1023) * Vin ; //analogo a voltaje
 float RLDR = (Rc * (Vin - Vout))/Vout; //voltaje a resistencia
 int lux_ldr = RDark/(RLDR/1000);//Conversión de resistencia a lux
 ...
 */
bool checkMeasures(float t, float h, int i) {
  // Se comprueba si ha habido algún error en la lectura
    ...
    if(isnan(i)){
      Serial.println("Error obteniendo los datos del sensor LDR");
      return false;
    }
    return true;
}
